...
public class ItemWandOfRetrieval extends Ebon3DItems
{
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int l)
    {
        ...
        if (i1 == Block.mobSpawner.blockID && !EbonAPI.doesPlayerHaveMagicExhaustion() && EbonAPI.hasNecroTome())
        {
            ...
            if (entityID != 0 && !EbonAPI.blacklistedSpawners.contains(entityString) && ((ItemNecroTome)EbonAPI.getNecroTome().getItem()).damageItemWithNotify(EbonAPI.getNecroTome(), 1, entityplayer))
            {
