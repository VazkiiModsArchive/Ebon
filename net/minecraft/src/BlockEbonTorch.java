...
public class BlockEbonTorch extends BlockTorch implements ITextureProvider
{
    ...
    public void randomDisplayTick(World world, int i, int j, int k, Random random)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            if (!(entityliving instanceof EntityPlayer) && random.nextInt(6) == 0 && !isIndirectlyPowered(world, i, j, k, l))
            {
                if (entityliving.getHealth() == 1)
                {
                    if (!mod_Ebon.markedEntities.contains(entity))
                    {
                        ...
                        world.spawnEntityInWorld(new EntityXPOrb(world, entityliving.posX, entityliving.posY, entityliving.posZ, entityliving.experienceValue / 2));
                    }
                }
